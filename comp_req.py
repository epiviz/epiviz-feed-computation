...
from ttest_gene import Ttest_Gene
import json
import itertools
import math
...
def comp_req(start_seq, end_seq, chromosome, gene_name, measurements=None):
    ...
    expression_data = gene_func.gene_data()
    ...
    if has_block:
        ...
        yield block_overlap
    if has_methy_diff:
        methy_raw_diff = Data_Functions(start_seq, end_seq, chromosome,measurements = methylation_diff_types).methylation_diff()
    ...
    if has_methy:
        ...
        methy_raw = Data_Functions(start_seq, end_seq, chromosome,measurements = methylation_types).methylation()
